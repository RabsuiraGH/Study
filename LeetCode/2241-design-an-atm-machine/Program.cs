...
and then be unable to use banknotes to complete the remaining $100. Note that the machine is not allowed
to use the $200 banknotes instead of the $500 banknote.
...
public class ATM
{
    private int[] banknotes = new int[5] { 0, 0, 0, 0, 0 };
    private int[] banknotesDenomination = { 20, 50, 100, 200, 500 };
    ...
    {
        var withdraw = new int[5];

        for (int i = withdraw.Length - 1; i >= 0; i--)
        {
            int banknoteCount = amount / banknotesDenomination[i];

            if (banknoteCount == 0) continue;
            if (amount >= banknotesDenomination[i])
            {
                int maxWithdrawCount = banknotes[i] >= banknoteCount ? banknoteCount : banknotes[i];
                withdraw[i] += maxWithdrawCount;
                amount -= withdraw[i] * banknotesDenomination[i];
            }
        }
        if (amount is > 0 or < 0) return new int[] { -1 };
